...
export const Route = createRootRouteWithContext<{ queryClient: QueryClient }>()({
  head: () => ({
    meta: [
      ...
      { title: "Lovable App" },
      { name: "description", content: "Cosmic Rewards Journey is a 100-level memory card game where players earn real-world rewards." },
      { name: "author", content: "Lovable" },
      { property: "og:title", content: "Lovable App" },
      { property: "og:description", content: "Cosmic Rewards Journey is a 100-level memory card game where players earn real-world rewards." },
...
function RootComponent() {
  ...
  useEffect(() => {
    registerPWA();
    try { const s = loadState(); startBackgroundMusic(s.muted, s.volume ?? 0.2); } catch { /* ignore */ }
  }, []);
